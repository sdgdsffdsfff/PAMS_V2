...
@InjectName("partyduebaseService")
@IocBean(args = { "refer:dao" }) 
public class PartyDueBaseService extends SkynetNameEntityService<PartyDueBase>
{
	public PartyDueBaseService()
	{
		super();
	}
	
	public PartyDueBaseService(Dao dao)
	{
		super(dao);
	}	
	
	public PartyDueBaseService(Dao dao, Class<PartyDueBase> entityType)
	{
...
//		sql.append(" select org.id deptid, org.cname deptname, '' baseuser, '' baseusername, base, base1, base2, base3, base4, base5 ").append("\n");
//		sql.append("  from t_sys_organ org ").append("\n");
//		sql.append("  left join ").append("\n");
//		sql.append(" ( ").append("\n");
//		sql.append(" select deptid, deptname, sum(base) base, sum(base1) base1, sum(base2) base2, sum(base3) base3, sum(base4) base4, sum(base5) base5 ").append("\n");
//		sql.append("  from t_app_pdbase base, t_app_pdbasedetail basedetail").append("\n");
//		sql.append(" where 1 = 1 ").append("\n");
//		sql.append("   and base.id = basedetail.baseid ").append("\n");
//		sql.append("   and base.id = ").append(SQLParser.charValue(baseid)).append("\n");
...
//		sql.append("   and org.parentorganid = ").append(SQLParser.charValue(orgid)).append("\n");
		...
		sql.append(" select org.id deptid, org.internal, org.cname deptname, usernums, sum(base) base, sum(base1) base1, sum(base2) base2, sum(base3) base3, sum(base4) base4, sum(base5) base5 ").append("\n");
		sql.append("  from (").append("\n");
		...
		sql.append(" left join ").append("\n");
		sql.append(" ( ").append("\n");
		sql.append(" select org.id deptid, org.internal, org.cname deptname, sum(base) base, sum(base1) base1, sum(base2) base2, sum(base3) base3, sum(base4) base4, sum(base5) base5 ").append("\n");
		sql.append("  from t_app_pdbase base, t_app_pdbasedetail basedetail, t_sys_organ org ").append("\n");
		sql.append(" where 1 = 1 ").append("\n");
		sql.append("   and base.id = basedetail.baseid ").append("\n");
		sql.append("   and basedetail.deptid = org.id ").append("\n");
		sql.append("   and base.id = ").append(SQLParser.charValue(baseid)).append("\n");
	...
	public List<DynamicObject> browsedeptbasedetails(String baseid, String deptid) throws Exception
	{
		...
		sql.append("    and orgb.id = gu.groupid ").append("\n");
		sql.append("    and (orgb.internal like orga.internal || '%') ").append("\n");
		sql.append("    and orga.id = ").append(SQLParser.charValue(deptid)).append("\n");
