...
jQuery(function($) {
	//获得数据
	$.ajax({
		url: '${base}/party/partydue//collect/collect/browseplan/json.action',
		dataType: 'json',
		success: function(d)
		{
			data = d;
...
{
	url = "${base}/party/partydue//collect/collect/startplan.action?planid=" + planid + "&flowdefid=" + flowdefid;
	window.location = url;
}
