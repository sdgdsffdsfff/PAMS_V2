...
public class SearchItemMake
{
	public static void clear(SearchVO vo)
	{
		String searchid = vo.searchid;
		clear(searchid);
	}
	...
	public static void searchitem(SearchVO vo)
	{
		...
		{
			sql = new StringBuffer();
			sql.append(" insert into t_sys_searchitem (searchitemid, searchid, caption, field, htmlfield, qfield, edittype, oorder)");
			sql.append(" values('" + (searchid + FormatKey.format(i+1, 2)) + "', '" + searchid + "', '" + vo.cnames[i] + "', '" + vo.names[i] + "', '" + vo.names[i] + "','" + vo.names[i] + "','text'," + (i+1) + ")");
			sql.append(";");
			System.out.println(sql.toString());
		}
		...
		if(vo.parentkey!=null)
		{
			...
			sql.append(" values('" + (searchid + FormatKey.format(vo.pkeys.length+1, 2)) + "', '" + searchid + "', '" + vo.parentkey + "', '" + vo.parentkey + "', '" + vo.parenthtmlkey + "','" + vo.parentkey + "', 'text', 0)");
			sql.append(";");
			System.out.println(sql.toString());
		}
	}
