...
public class PartyDueCollectBudgetService extends SkynetNameEntityService<PartyDueCollectBudget>
{
	...
	public PartyDueCollectBudgetService(Dao dao)
	{
		super(dao);
	}	
	...
	public List<DynamicObject> browsesumdeptbasedetails(String baseid, String orgid) throws Exception
	{
		...
		sql.append(" select org.id deptid, org.internal, org.cname deptname, usernums, sum(case when collcost1 is null then 0 else collcost1 end) collcost1, sum(case when base is null then 0 else base end ) base ").append("\n");
		sql.append("  from (").append("\n");
		...
		sql.append(" ( ").append("\n");
		sql.append(" select orga.id, orga.internal, orga.cname, sum(base) base ").append("\n");
		sql.append("  from t_app_pdbase base, t_app_pdbasedetail basedetail, t_sys_organ orga, t_sys_organ orgb ").append("\n");
		sql.append(" where 1 = 1 ").append("\n");
		sql.append("   and base.id = basedetail.baseid ").append("\n");
		sql.append("   and basedetail.deptid = orgb.id ").append("\n");
		sql.append("   and (orgb.internal like orga.internal || '%') ").append("\n");
		sql.append("   and orga.parentorganid = ").append(SQLParser.charValue(orgid)).append("\n");
		sql.append("  group by orga.id, orga.internal, orga.cname ").append("\n");
		...
		sql.append(" left join ").append("\n");
		sql.append(" ( ").append("\n");
		sql.append(" select orga.id, orga.internal, orga.cname, sum(collcost1) collcost1 ").append("\n");
		sql.append("  from t_app_pdcollbudget base, t_app_pdcollbudgetdetail basedetail, t_sys_organ orga, t_sys_organ orgb ").append("\n");
		sql.append(" where 1 = 1 ").append("\n");
		sql.append("   and base.id = basedetail.collbudgetid ").append("\n");
		sql.append("   and basedetail.colldeptid = orgb.id ").append("\n");
		sql.append("   and base.id = ").append(SQLParser.charValue(baseid)).append("\n");
		sql.append("   and (orgb.internal like orga.internal || '%') ").append("\n");
		sql.append("   and orga.parentorganid = ").append(SQLParser.charValue(orgid)).append("\n");
		sql.append("  group by orga.id, orga.internal, orga.cname ").append("\n");
		sql.append(" ) budget ").append("\n");
		sql.append("  on budget.internal like (org.internal || '%') ").append("\n");
		sql.append(" where 1 = 1 ").append("\n");
