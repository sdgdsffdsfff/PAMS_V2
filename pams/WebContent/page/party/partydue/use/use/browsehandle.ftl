...
jQuery(function($) {
	//获得数据
	$.ajax({
		url: '${base}/party/partydue/use/use/browsehandle/json.action',
		dataType: 'json',
		success: function(d)
		{
			data = d;
			...
			$.each(data,function(i,n)
			{
				...
				trHtml += '<td><a href="${base}/party/partydue/use/use/readpage.action?runactkey='+n.runactkey+'">'+n.cno+'</a></td>';
...
function page_locate(runactkey)
{
	...
	window.location = "${base}/party/partydue/use/use/readpage.action?runactkey=" + runactkey;
}
